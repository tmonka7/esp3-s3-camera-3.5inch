...
static esp_err_t panel_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(lcd_new_panel_st7796(s_io_handle, &panel_cfg, &s_panel_handle),
                        TAG, "st7796");
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_swap_xy(s_panel_handle, BSP_LCD_SWAP_XY),
                        TAG, "swap_xy");
...
static esp_err_t touch_init(void)
{
    ...
    const touch_ft6336_config_t tp_cfg = {
        .x_max    = BSP_LCD_H_RES,
        .y_max    = BSP_LCD_V_RES,
        .rst_gpio = BSP_TOUCH_PIN_RST,
        .int_gpio = BSP_TOUCH_PIN_INT,
        .swap_xy  = BSP_LCD_SWAP_XY,
        .mirror_x = BSP_LCD_MIRROR_X,
        .mirror_y = BSP_LCD_MIRROR_Y,
    };
    return touch_ft6336_init(&tp_cfg);
}
...
static esp_err_t lvgl_init(void)
{
    const lvgl_port_cfg_t port_cfg = {
        ...
        /* The panel already applies swap/mirror, so LVGL draws straight into
         * the rotated resolution with no software rotation. */
        .hres          = BSP_UI_H_RES,
        .vres          = BSP_UI_V_RES,
        ...
        .buffer_pixels = BSP_LCD_H_RES * 80,
        .task_priority = 4,
        .task_stack    = 8192,
        .task_core     = 0,     /* core 0; the camera pipeline owns core 1 */
    };
    ESP_RETURN_ON_ERROR(lvgl_port_init(&port_cfg), TAG, "lvgl_port_init");

    s_disp = lvgl_port_disp();
    ESP_RETURN_ON_FALSE(s_disp, ESP_FAIL, TAG, "no display");
    return ESP_OK;
}
